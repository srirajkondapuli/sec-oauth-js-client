...
export interface TokenRequestHandler {
   ...
   */
  performTokenRequest(configuration: AuthorizationServiceConfiguration,
                      request: TokenRequest): Promise<TokenResponse>;

  performRevokeTokenRequest(configuration: AuthorizationServiceConfiguration,
...
export class BaseTokenRequestHandler implements TokenRequestHandler {
  constructor(public readonly requestor: Requestor = new JQueryRequestor(),
              public readonly utils:
                  QueryStringUtils = new BasicQueryStringUtils()) {}
  ...
  performRevokeTokenRequest(configuration: AuthorizationServiceConfiguration,
    ...
    return revokeTokenResponse.then(response => { return true; });
  }
