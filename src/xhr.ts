...
export class JQueryRequestor extends Requestor {
  xhr<T>(settings: JQueryAjaxSettings): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      xhr.then(
          (data, textStatus, jqXhr) => { resolve(data as T); },
          (jqXhr, textStatus, error) => { reject(new AppAuthError(error)); });
    });
  }
}
    ...
    return fetch(url.toString(), requestInit).then(response => {
      if (response.status >= 200 && response.status < 300) {
        ...
        return Promise.reject(
            new AppAuthError(response.status.toString(), response.statusText));
      }
    });
  }
}
...
export class TestRequestor extends Requestor {
  constructor(public promise: Promise<any>) { super(); }
  xhr<T>(settings: JQueryAjaxSettings): Promise<T> {
