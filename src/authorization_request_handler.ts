...
export type AuthorizationListener =
    (request: AuthorizationRequest, response: AuthorizationResponse|null,
...
export class AuthorizationNotifier {
   ...
   */
  onAuthorizationComplete(request: AuthorizationRequest,
                          response: AuthorizationResponse|null,
                          error: AuthorizationError|null): void {
    if (this.listener) {
...
export const BUILT_IN_PARAMETERS =
    [ 'redirect_uri', 'client_id', 'response_type', 'state', 'scope' ];
...
export abstract class AuthorizationRequestHandler {
  ...
  completeAuthorizationRequestIfPossible(): Promise<void> {
    ...
    return this.completeAuthorizationRequest().then(result => {
      ...
      if (result && this.notifier) {
        this.notifier.onAuthorizationComplete(result.request, result.response,
                                              result.error);
      }
    });
  }
   ...
   */
  abstract performAuthorizationRequest(configuration:
                                           AuthorizationServiceConfiguration,
                                       request: AuthorizationRequest): void;

  /**
   * Checks if an authorization flow can be completed, and completes it.
   * The handler returns a `Promise<AuthorizationRequestResponse>` if ready, or
   * a `Promise<null>` if not ready.
   */
  protected abstract completeAuthorizationRequest():
