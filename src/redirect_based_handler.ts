...
} from './authorization_service_configuration';
...
const authorizationRequestKey =
    (handle: string) => { return `${handle}_appauth_authorization_request`; }
...
export class RedirectRequestHandler extends AuthorizationRequestHandler {
  ...
  performAuthorizationRequest(configuration: AuthorizationServiceConfiguration,
    ...
    // before you make request, persist all request related data in local
    // storage.
    const persisted = Promise.all([
      this.storageBackend.setItem(AUTHORIZATION_REQUEST_HANDLE_KEY, handle),
      // Calling toJson() adds in the code & challenge when possible
      request.toJson().then(
          result => this.storageBackend.setItem(authorizationRequestKey(handle),
                                                JSON.stringify(result))),
      this.storageBackend.setItem(authorizationServiceConfigurationKey(handle),
  ...
  protected completeAuthorizationRequest():
    ...
    return this.storageBackend.getItem(AUTHORIZATION_REQUEST_HANDLE_KEY)
        .then(handle => {
          if (handle) {
            ...
            return this.storageBackend
                ...
                .then(request => {
                  ...
                  let currentUri = `${this.locationLike.origin}${
                      this.locationLike.pathname}`;
                  let queryParams =
                      this.utils.parse(this.locationLike, true /* use hash */);
                  ...
                  log('Potential authorization request ', currentUri,
                      queryParams, state, code, error);
